...
def read_examples(dataset, vocabulary) -> Union[List[Example], None]:
    """Read examples from filename."""
    examples=[]
    with open('summarization\\clustering_tools.pkl', 'rb') as f:
        vectorizer, clustering_model = pickle.load(f)
    for repo in list(set(dataset['repo'].tolist())):
        ...
        for file_path in dataset_repo['file'].tolist():
                ...
                sentences = [doc.strip() for doc in sentences if len(doc.split())>3
                             and any(map(str.isdigit, doc)) is False
                             and doc.strip()[0] == doc.strip()[0].capitalize()]
                if len(sentences) == 0:
                    continue
                tfifdf_sentences = vectorizer.transform(sentences)
                labels = clustering_model.predict(tfifdf_sentences)
                docstring = ". ".join([sentence
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        source_tokens_spe =[tokenizer.cls_token]+source_tokens+[tokenizer.sep_token]
        source_ids = tokenizer.convert_tokens_to_ids(source_tokens_spe)
        source_mask = [1] * (len(source_tokens_spe))
        padding_length = args.max_source_length - len(source_ids)
        source_ids_padded = source_ids + [tokenizer.pad_token_id]*padding_length
        source_mask_padded =source_mask + [0]*padding_length
        #target
        if stage=="test":
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default="C:\ds\senatus-code-s4\codebert-base", type=str,
    ...
    ## Other parameters
    parser.add_argument("--train_filename", default='C:\ds\senatus-code-s4\dataset\\10_topics_train_dataset.pkl', type=str,
                        help="The train filename. Should contain the csv files for this task.")
    parser.add_argument("--dev_filename", default='C:\ds\senatus-code-s4\dataset\\10_topics_test_dataset.pkl', type=str,
    ...
    if args.do_train:
        ...
        with open(args.train_filename, 'rb') as f:
            train_dataset = pickle.load(f)
        with open('summarization\\vocabulary.json', 'r') as f:
            vocabulary = [voc for vocs in list(json.load(f).values()) for voc in vocs]
        train_examples = read_examples(train_dataset, vocabulary)
        ...
        logger.info("  Num epoch = %d", num_train_optimization_steps*args.train_batch_size//len(train_features))
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    with open(args.dev_filename, 'rb') as f:
                        eval_dataset = pickle.load(f)
                    eval_examples = read_examples(eval_dataset, vocabulary)
                ...
                if 'dev_bleu' in dev_dataset:
                    ...
                    eval_examples = read_examples(eval_dataset)
                ...
                with open(os.path.join(args.output_dir,"dev_output.txt"),'w') as f, open(os.path.join(args.output_dir,"dev_gold.txt"),'w') as f1:
                    for ref,gold in zip(p,eval_examples):
                        predictions.append(str(gold.idx)+' '+str(gold.repo)+'\t'+ref)
                        f.write(str(gold.idx)+' '+str(gold.repo)+'\t'+ref+'\n')
                        f1.write(str(gold.idx)+' '+str(gold.repo)+'\t'+gold.target+'\n')
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):
            ...
            with open(file, 'rb') as f:
                example_file = pickle.load(f)
            eval_examples = read_examples(example_file, vocabulary)
            eval_features = convert_examples_to_features(eval_examples, tokenizer, args,stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            all_source_mask = torch.tensor([f.source_masks for f in eval_features], dtype=torch.long)
            eval_data = TensorDataset(all_source_ids,all_source_mask)
